...
namespace IP2WhoisComponent
{
	public class IP2Whois
	{
		private const string SOURCE = "sdk-dotnet";
		private const string VERSION = "1.0.0";
		private const string BASE_URL = "https://api.ip2whois.com/v2";
		private const string FORMAT = "json";
		private const string ERROR_WHOIS = "WHOIS lookup error.";
		private const string ERROR_DOMAIN = "Domain not found.";
		...
		public async Task<JObject> Lookup(string Domain)
		{
			try
			{
				Url = BASE_URL + "?format=" + FORMAT + "&source=" + SOURCE + "&source_version=" + VERSION + "&key=" + APIKey + "&domain=" + WebUtility.UrlEncode(Domain ?? "");
				...
				else if ((Response.StatusCode == HttpStatusCode.Unauthorized) || (Response.StatusCode == HttpStatusCode.BadRequest))
				{
					...
					throw new Exception(ERROR_WHOIS);
				}
			}
		...
		// Get Punycode
		public static string GetPunycode(string domain)
		{
			IdnMapping idn = new IdnMapping();
			string punycode = idn.GetAscii(domain);
			return punycode;
		}

		// Get normal text
		public static string GetNormalText(string domain)
		{
			IdnMapping idn = new IdnMapping();
			string normal = idn.GetUnicode(domain);
			return normal;
		}
		...
		// Description: Get domain extension from URL/domain
		public static string GetDomainExtension(string str)
		{
